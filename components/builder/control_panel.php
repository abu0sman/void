<?php
//Выясняем а не открыли ли мы при помощи не предназначенного для этого комонента, архивный ордер
//Control Panel
echo '<div id="control_panel" class="form-inline">';
	...
	//Если задача только что стала архивной
	//if ($current_order_data[0]['state'] != '200'){
	//if (($group_id == 1) OR ($ugroup == $slave_id)){		
		echo '<div id="cp_separator">&nbsp</div>' ;
		
		echo '<button id="b_add_component" class="btn btn-outline-success btn-sm mr-1" title="Создание таблицы из csv файла">
		<span class="ui-icon ui-icon-pencil"></span>CSV</button>';
		
		
		//echo '<form method="POST">';
			echo '<button id="b_edit_order" class="btn btn-outline-success btn-sm mr-1" title="Редактировать ордер">
		...
		//Кнопка "Удалить"
		/* echo '<button id="b_arch_order" class="btn btn-outline-warning btn-sm mr-1" title="Переместить ордер в архив">
