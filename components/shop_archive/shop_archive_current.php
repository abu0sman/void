...
$reference = sql_referenser_s('../../db_connect.php', 'm_orders', $count_on_page, $actual_page, $SearchCol, $SearchRow, 'ARCH');
$count_pages = $reference[0];
$pstart = $reference[2];
$sql_constructor_result = sql_constructor_s('../../db_connect.php', 'm_orders', $pstart, $count_on_page, $SearchCol, $SearchRow, 'ARCH');
...
foreach($result_main as $row){
	...
	echo '<form method="POST">';
		echo '<button name="page_name" value="show_archive" class="card m-2 btn btn-light" id="butt'. $oid .'" style="display: block; width:100%; text-align:left;">';
...
echo '<script src="../shop_archive/shop_scripts.js"></script>';
echo "<script>$('.prev_page').on('click', function(){
	var c_page =". $actual_page .";
	p_page = c_page-1;
	$.ajax({
		data:	{ page_id: p_page, page_name: 'shop_archive' },
...
$('.next_page').on('click', function(){
	...
	/* alert(n_page); */
	$.ajax({
		data:	{page_id: n_page, page_name: 'shop_archive'},
		url:	'../shop_archive/shop_archive_current.php',
		type:	'POST',
		cache:	false,
		success:function(result){
