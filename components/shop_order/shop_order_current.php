...
if (isset($_POST['ch_pe'])){
	$_SESSION['actual_page'] = $_POST['page_id'];
	$actual_page = $_SESSION['actual_page'];
}
else {//Если не задана страница
...
$reference = sql_referenser_s('../../db_connect.php', 'm_orders', $count_on_page, $actual_page, $SearchCol, $SearchRow);
$count_pages = $reference[0];
$pstart = $reference[2];
$sql_constructor_result = sql_constructor_s('../../db_connect.php', 'm_orders', $pstart, $count_on_page, $SearchCol, $SearchRow);
...
foreach($result_main as $row){
	...
	echo '<form method="POST">';
		echo '<button name="page_name" value="show_order" class="card m-2 btn btn-light" id="butt'. $oid .'" style="display: block; width:100%; text-align:left;">';
...
echo '<script src="../shop_order/shop_scripts.js"></script>';

echo "<script>$('.prev_page').on('click', function(){
	var c_page =". $actual_page .";
	p_page = c_page-1;
	$.ajax({
		data:	{ page_id: p_page, page_name: 'shop_order' },
...
$('.next_page').on('click', function(){
	...
	n_page = c_page+1;
	$.ajax({
		data:	{ page_id: n_page, page_name: 'shop_order' },
		url:	'../shop_order/shop_order_current.php',
		type:	'POST',
		cache:	false,
		success:function(result){
